    ...
    <Card>
      ...
      <CardContent className="space-y-3">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" className="flex-1 text-[12px]" onClick={selectAllGroups}>
            <CheckSquare className="h-3 w-3" /> All
          </Button>
          <Button variant="outline" size="sm" className="flex-1 text-[12px]" onClick={clearGroups}>
            <Square className="h-3 w-3" /> None
          </Button>
        </div>

        <div className="max-h-48 overflow-y-auto rounded-lg border border-ink-200 bg-page divide-y divide-ink-200">
          {filtered.map(g => (
            <button
              key={g.id}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="font-medium text-ink-700 truncate">{g.name}</p>
                <p className="text-[11px] text-ink-400">{g.city} · {(g.memberCount/1000).toFixed(1)}k members</p>
              </div>
              <span className="text-[11px] text-ink-400">{CATEGORY_LABELS[g.category]}</span>
            </button>
          ))}
        </div>

        <div className="rounded-lg bg-purple-500/10 border border-purple-500/20 p-2.5 text-[12px] text-ink-400">
          Est. reach: <span className="font-semibold text-purple-500">{(totalReach/1000).toFixed(0)}k members</span>
        </div>
