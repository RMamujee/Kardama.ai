...
export function getYtdRevenue(payments: Payment[]): number {
  ...
  return payments.filter(p => p.status === 'confirmed' && p.month.startsWith(year)).reduce((s, p) => s + p.amount, 0)
}

export function getMtdRevenue(payments: Payment[]): number {
  const month = new Date().toISOString().slice(0, 7)
  return payments.filter(p => p.status === 'confirmed' && p.month === month).reduce((s, p) => s + p.amount, 0)
}
...
export function getMtdTrend(payments: Payment[]): string | null {
  ...
  const currAmt = confirmed.filter(p => p.month === curr).reduce((s, p) => s + p.amount, 0)
  const prevAmt = confirmed.filter(p => p.month === prev).reduce((s, p) => s + p.amount, 0)
...
export function getYtdTrend(payments: Payment[]): string | null {
  ...
  const confirmed = payments.filter(p => p.status === 'confirmed')
  const currAmt = confirmed.filter(p => p.month.startsWith(currYear)).reduce((s, p) => s + p.amount, 0)
  const prevAmt = confirmed.filter(p => p.month.startsWith(prevYear)).reduce((s, p) => s + p.amount, 0)
  if (prevAmt === 0) return null
  const pct = ((currAmt - prevAmt) / prevAmt) * 100
  return `${pct >= 0 ? '+' : ''}${pct.toFixed(0)}%`
}
